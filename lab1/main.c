...
BN square(BN num) {
    ...
    {
        uv = (DBASE)res.coef[2 * i] + (DBASE)num.coef[i] * (DBASE)num.coef[i];// пункт 2.1
...
int main() {
    ...
    cin >> bn1;
     cout << bn1 << endl;
     time_point<system_clock> begin1 = system_clock::now();
     BN bn2 = bn1*bn1;
     ...
     time_point<system_clock> begin2 = system_clock::now();
     BN bn3 = square(bn1);
